...
8. A subset of a metric space is **bounded** if it is contained in some open ball. Similarly, a sequence is bounded iff its term set is a bounded set. In this case we can define the **diameter** of a bounded set to be the supremum of the set of distances between any two points in the set. (We can also define the diameter of unbounded sets to be $\infty$ using the extended reals)

9. A Cauchy sequence is bounded. There is a tail sequence for which every term is within $1$ of one another, so pick the first term in the tail sequence, $x_N$. Then an open ball of radius $\max \{d(x_0, x_N), \ldots, d(x_{N-1}, x_N, 1\}$ contains the whole set.

10. A map $f: X \to Y$ between metric spaces is **continuous** at $a$ if for every open ball in $Y$ around $f(a)$, there is an open ball in $X$ around $a$ that $f$ maps into the first open ball. This is the standard $\epsilon-\delta$ definition in different clothes. See the topology notes for further deets.

11. One interesting characterization of continuous maps is as *locally approximately constant* maps. (The following is non-standard. I have not seen it anywhere, I made it up). Let's say that a map $f$ is **locally approximately constant at $a$ of tolerance $\epsilon$** ($\epsilon > 0$) if there is a neighborhood around $a$ such that when $f$ is restricted to that neighborhood, the output is always within $\epsilon$ of $f(a)$.
...
12. An **isometry** is any map between metric spaces that preserves distance: $d_X(a, b) = d_Y(f(a), f(b))$ for all $a, b \in X$. Every isometry $f$ is a topological embedding:
...
13. Different metrics on the same set can nevertheless induce the same metric topologies. We say that two such metrics are **topologically equivalent**. Metrics are topologically equivalent iff the open balls *nest*, meaning for every $x \in X$ and every $\epsilon > 0$, there are $\delta, \gamma > 0$ such that
...
17. Here's a sequential characterization of closed subsets that are useful: a subset $S$ of a metric space is closed iff every sequence in $S$ that converges in $X$ has its limit in $S$.
...
18. A function $f: X \to Y$ between metric spaces is **uniformly continuous** if for every $\epsilon > 0$, there's a $\delta > 0$ such that for *all* $x \in X$, $f(B(x; \delta)) \subseteq B(f(x); \epsilon)$. The difference from mere continuity is that the same $\delta$ works for *every $x$* in uniform continuity, whereas $\delta$ is dependent on $x$ under continuity.

19. A function $f: X \to Y$ between metric spaces is **Lipschitz continuous** with **Lipschitz constant $K \geq 0$** when $d_Y(f(a), f(b)) \leq K d_X(a, b)$ for all $a, b \in X$.

20. Every Lipschitz continuous map is uniformly continuous, since for any $\epsilon$ and any $x \in X$, if $d_X(a, x) < \epsilon/K$ then $d_Y(f(a), f(x)) < \epsilon$.

21. A **contraction map** is any Lipschitz continuous map where the domain and the codomain are equal and the Lipschitz constant $K$ has $0 \leq K < 1$.

22. If $X, Y$ are metric spaces, $X$ compact and $f: X \to Y$ continuous, then $f$ must additionally be uniformly continuous. *Proof:* Since $f$ is continuous, for every $\epsilon$ and $x \in X$ there's a $\delta_x$ such that
...
23. Two open balls $B(x; r)$ and $B(y; s)$ are disjoint when $r+s \leq d(x,y)$. This is because

    $$d(x,y) \leq d(x, a) + d(a, y)$$

    for any $a$, so if there were some $a$ in both balls we'd have $d(x, y) < r + s$.

24. One open ball $B(x; r)$ is contained in another $B(y; s)$ when $d(x,y) \leq s - r$. We reason again by the triangle inequality: for all $a \in B(x; r)$, we have
